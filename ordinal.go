...
// ordinalOnes maps ones digits 1-9 to ordinal forms [masculine, feminine, neuter].
var ordinalOnes = [][3]string{
...
// ordinalTeens maps teens 10-19 (indexed by last digit 0-9) to ordinal forms.
var ordinalTeens = [][3]string{
...
// ordinalTens maps tens 20-90 (index 0=20, 1=30, ..., 7=90) to ordinal forms.
var ordinalTens = [][3]string{
...
// ordinalHundreds maps 100-900 (index 0=100, ..., 8=900) to ordinal forms.
var ordinalHundreds = [][3]string{
	{"сотый", "сотая", "сотое"},
...
// ordinalOrders maps order indices 1-13 to ordinal forms for round order numbers.
var ordinalOrders = [][3]string{
...
// onesCompound provides compound prefixes for digits 1-9 when forming
// compound ordinal numbers like "двухтысячный".
var onesCompound = []string{
...
func ordinalTriad(n int, gender Gender) string {
	h := n / 100
	t := (n % 100) / 10
	o := n % 10
	gi := genderIndex(gender)
...
func roundOrdinal(prefix int, gender Gender) string {
	...
	// Find the highest order. prefix represents thousands, so we need to find
	// which order level this is. We decompose prefix into triads.
	// The order level: prefix has its own order structure.
	// 1 (prefix=1) => 1000 => order 1 (тысячный)
	// 1000 (prefix=1000) => 1000000 => order 2 (миллионный)
	// etc.
	...
	// orderLevel is how many more factors of 1000 there are in prefix
	// The actual order of the original number is orderLevel + 1 (because prefix = n/1000)
	// But we need to handle the coefficient (temp)
